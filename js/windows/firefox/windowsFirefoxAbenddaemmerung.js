(function () {
    ...
    //neue Farben
    let newBackgroundColorA = 'rgb(45, 50, 54)';
    let newColorA = 'rgb(255, 255, 255)';
    let newLinkColor = 'rgb(112, 235, 222)';
    let newLInkColorVisited ='rgb(150, 255, 155)';
    setLinkfarben();
    ...
    let firefoxFarben = TestColors();
        ...
        if (firefoxFarben[1] == true) {
            newBackgroundColorA = aktualBackground;
        }
        if (firefoxFarben[2] == true) {
            newColorA = aktualColor;
        }
    

    //als Child-Element hinzu. Dabei beinhalten die Variablen die jeweiligen Farbwerte für den Browser

    var css = 'body{background-color:'+newBackgroundColorA+';color:'+newColorA+';}';
    frame(document);
...
function TestLinkColors(elements) {
    //aktual colors
    let aktualColor = window.getComputedStyle(elements, null).getPropertyValue('color');
    let rgbColor = RgbWerte(aktualColor);

    //Linkfarbe visited in Firefox standardmäßig:
    let standardVisited = 'rgb(64, 0, 64)';
    let standardVisitedCE ='rgb(85, 26, 139)';
    let standardLinkCE = 'rgb(0, 0, 238)';
    let standardLink = 'rgb(0, 102, 204)';

    let colorUnset;

    /*color auf Standardwert überprüfen */
    if (aktualColor == standardVisited || standardVisitedCE || standardLinkCE || standardLink) {
        colorUnset = true;
    } else {
        colorUnset = false;
    }
    ...
    //als Child-Element hinzu. Dabei beinhalten die Variablen die jeweiligen Farbwerte für den Browser  
    //set color in HTML but background-color must be set in body-Element to get the right results
    var css = 'a {color:' + newLinkColor + ';} a:visited {color:' + newLinkColor + ';} ';
    frame(document);
    ...
    function frame(d) {
        ...
        styleContent = d.createTextNode(css);
        style.appendChild(styleContent);
        d.getElementsByTagName(elements.nodeName)[0].prepend(style);
        var elem = d.querySelectorAll('iframe,frame');
        for (var i = 0; i < elem.length; i++) {
            d = elem[i].contentWindow.document;
            frame(d);
    ...
    let newAktualColor = window.getComputedStyle(element, null).getPropertyValue('color');
    ...
    //Change values
    let aktualStyle2 = document.getElementsByClassName('wcagstyle')[0];
    aktualStyle2.innerHTML = 'a {color:' + newLinkColor + ';} a:visited {color:' + newLinkColor + ';} ';
